<template>
  <div id="teacherIndex">
    ...
    <mu-list class="button-list">
      <mu-list-item>
        <mu-button color="primary"
                   to="/student/index"
                   class="all-button">查看所有房间</mu-button>
        <mu-button color="success"
                   @click="showAlert"
                   class="new-button">新建房间</mu-button>
      </mu-list-item>
    </mu-list>
    <mu-alert @delete="isAlert = false"
      ...
      <mu-paper class="paper"
        ...
        <mu-form class="form"
                 ...
                 label-width="80">
          <mu-row>
            <mu-text-field v-model="roomName"
                           ref="roomName"
                           label="房间名称"
                           class="name"></mu-text-field>
          ...
          <mu-row>
            <mu-form-item label="白板"
                          class="text-left">
              <mu-switch v-model="switchVal.isBoard"
                         ref="isBoard"></mu-switch>
            </mu-form-item>
            <mu-form-item label="代码编译区"
                          class="switch">
              <mu-switch v-model="switchVal.isCode"
                         ref="isCode"></mu-switch>
            </mu-form-item>
            <mu-form-item label="密码"
                          class="switch">
              <mu-switch v-model="switchVal.isPassword"
                         ref="isPassword"></mu-switch>
            </mu-form-item>
            <mu-text-field v-if="switchVal.isPassword===true"
                           v-model="password"
                           type="password"
                           ref="password"
                           label="房间密码"
                           class="password"></mu-text-field>
          </mu-row>
          <mu-row>
            <mu-text-field v-model="roomDescription"
                           multi-line
                           :rows="3"
                           :rows-max="6"
                           ref="roomDescription"
    ...
    <mu-list v-bind:style="style">
      <mu-list-item>暂无数据</mu-list-item>
    </mu-list>
    <mu-list class="list">
      <mu-list-item v-for="item in list"
                    :key="item.roomId">
        <span class="point">●</span> 房间&nbsp;{{item.roomId}}：{{item.name}}
        <mu-button v-bind:id="item.roomId"
                   color="primary"
                   @click="getRoomInfo($event)"
                   class="list-button1"
                   flat>编辑</mu-button>
        <mu-button color="success"
                   v-bind:id="item.roomId"
